...
def main():
    ...
    reference_acronyms = {}
    query_acronyms = {}
    # only need unique mapping of reference to acronym and query to acronym
    for reference in weighted_f1_results["reference"].unique():
        reference_acronyms[reference] = weighted_f1_results[weighted_f1_results["reference"] == reference]["reference_acronym"].iloc[0]
    
    for query in weighted_f1_results["query"].unique():
        query_acronyms[query] = weighted_f1_results[weighted_f1_results["query"] == query]["query_acronym"].iloc[0]
    ...
    heatmap_data = pd.pivot_table(
        weighted_f1_results, 
        values="weighted_f1", 
        index=["query_acronym", "study","sex", "dev_stage", "disease"],
        columns=["reference_acronym","cutoff","subsample_ref",'method',"ref_split"]
    )
    
    write_na_report(heatmap_data)
   # Create the column_metadata DataFrame based on heatmap data columns
    column_metadata = pd.DataFrame(heatmap_data.columns.tolist(), columns=["reference_acronym", "cutoff", "subsample_ref", "method","ref_split"])
    row_metadata = pd.DataFrame(heatmap_data.index.tolist(), columns=["query_acronym", "study","sex", "dev_stage", "disease"])

    # Define color palettes for each column category
    reference_acronym_colors = sns.color_palette("Set1", n_colors=len(column_metadata["reference_acronym"].unique()))
    ...
    study_colors = sns.color_palette("Set1", n_colors=len(row_metadata["query_acronym"].unique()))
    ...
    col_colors = pd.DataFrame({
        'Reference': column_metadata['reference_acronym'].map(dict(zip(column_metadata["reference_acronym"].unique(), reference_acronym_colors))),
...
def write_na_report(heatmap_data):
    # Check for NaN or infinite values
    if heatmap_data.isnull().values.any() or np.isinf(heatmap_data).any():
        warnings.warn("NaN or infinite values detected in the data. Replacing with 0.")
       #subset for Nas and write to file
        na_subset = heatmap_data[heatmap_data.isnull().values]
