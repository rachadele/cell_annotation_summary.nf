...
def plot_model_summary(model_summary):
    ...
    for key, subset in model_summary.groupby("key"):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(key,f"{key}_{formula}_lm_coefficients.png"))
...
def plot_contrast_boxplot(df, model, contrast, model_summary_df, outdir):
    """Plot a boxplot comparing two conditions based on a linear model contrast."""
    # Extract the contrast value and p-value
    contrast_value = model.params.get(contrast, None)
    # term is the index of the model_summary_df
    p_value = model_summary_df.loc[contrast:contrast]["FDR"].values[0]

    if contrast_value is None:
        print(f"Contrast {contrast} not found in model parameters.")
        return

    # Extract the two reference groups from the contrast string
    terms = contrast.split(":method[T.")
    if len(terms) == 2:
        reference_group = terms[0].replace("reference[T.", "").replace("]", "").strip()
        method_group = terms[1].replace("]", "").strip()
    else:
        reference_group = None
        method_group = terms[0].replace("method[T.", "").replace("]","").strip()
        
    baseline = "seurat"

    # Filter the DataFrame for the relevant groups
    if reference_group:
        df_filtered = df[df['reference'].isin([reference_group]) & df['method'].isin([baseline, method_group])]
    ...
    plt.plot([x1, x1, x2, x2], [y_max, y_max + 0.02, y_max + 0.02, y_max], color="black", lw=1.5)
...
def main():
    ...
    for df in df_list:
        ...
        os.makedirs(df["key"].values[0], exist_ok=True)
        outdir=df["key"].values[0]
        for formula in formulas:
            model, model_summary, model_summary_df = run_lm_regressed(df, formula)
            if "reference[T.whole cortex]:method[T.scvi]" in model_summary_df.index:
                contrast = "reference[T.whole cortex]:method[T.scvi]"
                plot_contrast_boxplot(df, model, contrast, model_summary_df, outdir)
            
            contrast = "method[T.scvi]"
            plot_contrast_boxplot(df, model, contrast, model_summary_df, outdir)
